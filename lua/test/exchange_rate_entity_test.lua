...
describe("ExchangeRateEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HUBSPOT_SETTINGS_TEST_EXCHANGE_RATE_ENTID JSON to run live")
      return
    end
    ...
    local exchange_rate_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.exchange_rate"), "exchange_rate_ref01"))

    local exchange_rate_ref01_data_result, err = exchange_rate_ref01_ent:create(exchange_rate_ref01_data, nil)
    assert.is_nil(err)
...
function exchange_rate_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("HUBSPOT_SETTINGS_TEST_EXCHANGE_RATE_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["HUBSPOT_SETTINGS_TEST_EXCHANGE_RATE_ENTID"])
