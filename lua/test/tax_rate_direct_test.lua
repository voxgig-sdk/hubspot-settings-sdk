...
describe("TaxRateDirect", function()
  it("should direct-list-tax_rate", function()
    local setup = tax_rate_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-tax_rate", setup.live and "live" or "unit")
  ...
  it("should direct-load-tax_rate", function()
    local setup = tax_rate_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-tax_rate", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
