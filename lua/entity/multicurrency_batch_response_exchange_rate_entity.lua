-- HubspotSettings SDK MulticurrencyBatchResponseExchangeRate entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local MulticurrencyBatchResponseExchangeRateEntity = {}
MulticurrencyBatchResponseExchangeRateEntity.__index = MulticurrencyBatchResponseExchangeRateEntity


function MulticurrencyBatchResponseExchangeRateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MulticurrencyBatchResponseExchangeRateEntity)
  self._name = "multicurrency_batch_response_exchange_rate"
  self._client = client
...
function MulticurrencyBatchResponseExchangeRateEntity:get_name()
  return self._name
end


function MulticurrencyBatchResponseExchangeRateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MulticurrencyBatchResponseExchangeRateEntity.new(self._client, opts)
end
...
function MulticurrencyBatchResponseExchangeRateEntity:mark_deleted()
  self._deleted = true
end


function MulticurrencyBatchResponseExchangeRateEntity:deleted()
  return true == self._deleted
end


function MulticurrencyBatchResponseExchangeRateEntity:data_set(args)
  if args ~= nil then
...
function MulticurrencyBatchResponseExchangeRateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MulticurrencyBatchResponseExchangeRateEntity:match_set(args)
  if args ~= nil then
...
function MulticurrencyBatchResponseExchangeRateEntity:match_get()
...
function MulticurrencyBatchResponseExchangeRateEntity:stream(action, args, callopts)
...
---@param reqdata MulticurrencyBatchResponseExchangeRateCreateData
---@param ctrl? table
---@return MulticurrencyBatchResponseExchangeRate
---@return string? err
function MulticurrencyBatchResponseExchangeRateEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function MulticurrencyBatchResponseExchangeRateEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return MulticurrencyBatchResponseExchangeRateEntity
