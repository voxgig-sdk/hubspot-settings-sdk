...
local TaxRateEntity = {}
TaxRateEntity.__index = TaxRateEntity


function TaxRateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TaxRateEntity)
  self._name = "tax_rate"
  self._client = client
...
function TaxRateEntity:get_name()
  return self._name
end


function TaxRateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TaxRateEntity.new(self._client, opts)
end
...
function TaxRateEntity:mark_deleted()
  self._deleted = true
end


function TaxRateEntity:deleted()
  return true == self._deleted
end


function TaxRateEntity:data_set(args)
  if args ~= nil then
...
function TaxRateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TaxRateEntity:match_set(args)
  if args ~= nil then
...
function TaxRateEntity:match_get()
...
function TaxRateEntity:stream(action, args, callopts)
...
---@param reqmatch TaxRateLoadMatch
---@param ctrl? table
---@return TaxRate
---@return string? err
function TaxRateEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch TaxRateListMatch
---@param ctrl? table
---@return TaxRate[]
---@return string? err
function TaxRateEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function TaxRateEntity:_run_op(ctx, post_done)
  local utility = self._utility
