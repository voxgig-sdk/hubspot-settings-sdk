# hubspot-settings-mcp

[MCP](https://modelcontextprotocol.io) server exposing the HubspotSettings SDK as
two agent tools — `hubspot-settings_list` and `hubspot-settings_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hubspot-settings-mcp)
make build
...
claude mcp add --scope user hubspot-settings \
  -- /absolute/path/to/hubspot-settings-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hubspot-settings-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hubspot-settings-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hubspot-settings \
     -- "$PWD"/dist/*/hubspot-settings-mcp -transport stdio
   ```
...
### Call the `hubspot-settings_list` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `hubspot-settings_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hubspot-settings_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./hubspot-settings-mcp -transport http -addr :18080 &
