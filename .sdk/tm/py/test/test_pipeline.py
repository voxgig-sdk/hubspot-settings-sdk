...
from projectname_sdk import HubspotSettingsSDK
from projectname_sdk.core.error import HubspotSettingsError
from projectname_sdk.core.result import HubspotSettingsResult
from projectname_sdk.core.response import HubspotSettingsResponse
from projectname_sdk.core.spec import HubspotSettingsSpec
from projectname_sdk.feature.base_feature import HubspotSettingsBaseFeature


def _client():
    return HubspotSettingsSDK.test(None, None)
...
def _resp(status, data=None, headers=None):
    ...
    return HubspotSettingsResponse({
        "status": status,
...
class TestMakePointAndMakeSpec:
    ...
    def test_make_spec_short_circuits_a_feature_supplied_spec(self):
        client = _client()
        ctx = _ctx(client, opname="load")
        preset = HubspotSettingsSpec({"method": "GET"})
        ctx.out["spec"] = preset
...
class TestMakeResponse:
    ...
    def test_guards_missing_spec_response_result(self):
        ...
        ctx.result = HubspotSettingsResult({})
...
class TestMakeResult:
    ...
    def test_list_op_wraps_resdata_into_entity_instances(self):
        ...
        ctx.entity = entity
        ctx.spec = _full_spec()
        ctx.result = HubspotSettingsResult({"resdata": [{"a": 1}, {"a": 2}]})
    ...
    def test_an_empty_list_yields_an_empty_resdata_array(self):
        ...
        ctx.spec = _full_spec()
        ctx.result = HubspotSettingsResult({"resdata": []})
        result, err = client._utility.make_result(ctx)
        assert err is None
    ...
    def test_short_circuits_on_a_preset_result(self):
        client = _client()
        ctx = _ctx(client)
        preset = HubspotSettingsResult({"ok": True})
...
class TestMakeRequest:
    ...
    def test_short_circuits_a_feature_supplied_request(self):
        client = _client()
        ctx = _ctx(client)
        preset = HubspotSettingsResponse({"status": 201, "statusText": "OK"})
...
class TestMakeErrorAndDone:
    ...
    def test_make_error_returns_resdata_when_throw_is_disabled(self):
        ...
        ctx.result = HubspotSettingsResult({"ok": False, "resdata": "fallback"})
...
class TestFeatureOrdering:
    ...
    def _named_feature(self, name):
        feature = HubspotSettingsBaseFeature()
        feature.name = name
        return feature
...
class TestPrepareAuth:
    ...
    def _auth_ctx(self, client, options, headers):
        ...
        ctx.spec = None if headers is None else HubspotSettingsSpec(
            {"headers": headers})
        return ctx
...
class TestResultHelpers:
    ...
    def test_result_headers_with_no_headers_yields_an_empty_map(self):
        client = _client()
        ctx = _ctx(client)
        ctx.response = HubspotSettingsResponse({"status": 200})
        ctx.result = HubspotSettingsResult({})
