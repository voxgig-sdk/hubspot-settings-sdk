...
// HubspotSettings SDK feature test
//
...
class FtCtrl extends HubspotSettingsControl
{
...
class FtClient
{
    ...
    private HubspotSettingsUtility $utility;

    public function __construct(HubspotSettingsUtility $utility, array $options)
    {
    ...
    public function get_utility(): HubspotSettingsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HubspotSettingsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HubspotSettingsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HubspotSettingsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HubspotSettingsConfig::make_feature($name);
    ...
    private static function buildUrl(HubspotSettingsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HubspotSettingsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HubspotSettingsResponse($fetched);
        }
    ...
    private function fail(HubspotSettingsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HubspotSettingsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
