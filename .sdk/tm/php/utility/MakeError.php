...
class HubspotSettingsMakeError
{
    public static function call(?HubspotSettingsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HubspotSettingsContext([], null);
        }
        $op = $ctx->op ?? new HubspotSettingsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HubspotSettingsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HubspotSettingsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new HubspotSettingsError('', $msg, $ctx);
        ...
        if ($err instanceof HubspotSettingsError) {
