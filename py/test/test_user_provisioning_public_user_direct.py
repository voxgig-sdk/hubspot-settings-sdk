...
class TestUserProvisioningPublicUserDirect:

    def test_should_direct_load_user_provisioning_public_user(self):
        setup = _user_provisioning_public_user_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-user_provisioning_public_user", "live" if setup["live"] else "unit")
        if _skip:
...
def _user_provisioning_public_user_direct_setup(mockres):
