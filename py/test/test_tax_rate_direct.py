...
class TestTaxRateDirect:

    def test_should_direct_list_tax_rate(self):
        setup = _tax_rate_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-tax_rate", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_tax_rate(self):
        setup = _tax_rate_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-tax_rate", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _tax_rate_direct_setup(mockres):
