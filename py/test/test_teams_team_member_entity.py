...
class TestTeamsTeamMemberEntity:

    def test_should_create_instance(self):
        testsdk = HubspotSettingsSDK.test(None, None)
        ent = testsdk.TeamsTeamMember(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _teams_team_member_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "teams_team_member." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HUBSPOT_SETTINGS_TEST_TEAMS_TEAM_MEMBER_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        teams_team_member_ref01_ent = client.TeamsTeamMember(None)
        ...
        teams_team_member_ref01_data = helpers.to_map(runner.entity_data(teams_team_member_ref01_ent.create(teams_team_member_ref01_data, None)))
        assert teams_team_member_ref01_data is not None



def _teams_team_member_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/teams_team_member/TeamsTeamMemberTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "HUBSPOT_SETTINGS_TEST_TEAMS_TEAM_MEMBER_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HUBSPOT_SETTINGS_TEST_TEAMS_TEAM_MEMBER_ENTID"))
