# HubspotSettings SDK test runner
...
class HubspotSettingsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HubspotSettingsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HubspotSettingsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HubspotSettingsTestRunner.getenv("HUBSPOT_SETTINGS_TEST_LIVE")
        override = HubspotSettingsTestRunner.getenv("HUBSPOT_SETTINGS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HubspotSettingsTestRunner.getenv(key)
        ...
        explain = HubspotSettingsTestRunner.getenv("HUBSPOT_SETTINGS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HUBSPOT_SETTINGS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HubspotSettingsTestRunner._test_control is not None:
            return HubspotSettingsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HubspotSettingsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HubspotSettingsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HubspotSettingsTestRunner.load_test_control()
...
def load_env_local():
    HubspotSettingsTestRunner.load_env_local()


def env_override(m):
    return HubspotSettingsTestRunner.env_override(m)


def entity_data(v):
    return HubspotSettingsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HubspotSettingsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HubspotSettingsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HubspotSettingsTestRunner.load_test_control()


def live_client_options():
    return HubspotSettingsTestRunner.live_client_options()


def live_delay_ms():
    return HubspotSettingsTestRunner.live_delay_ms()
