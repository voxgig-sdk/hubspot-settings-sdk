...
class TestMulticurrencyCollectionResponseExchangeRateForwardPagingDirect:

    def test_should_direct_list_multicurrency_collection_response_exchange_rate_forward_paging(self):
        setup = _multicurrency_collection_response_exchange_rate_forward_paging_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-multicurrency_collection_response_exchange_rate_forward_paging", "live" if setup["live"] else "unit")
        if _skip:
...
def _multicurrency_collection_response_exchange_rate_forward_paging_direct_setup(mockres):
