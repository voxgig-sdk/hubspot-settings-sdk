...
class TestTeamsTeamDirect:

    def test_should_direct_load_teams_team(self):
        setup = _teams_team_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-teams_team", "live" if setup["live"] else "unit")
        if _skip:
...
def _teams_team_direct_setup(mockres):
