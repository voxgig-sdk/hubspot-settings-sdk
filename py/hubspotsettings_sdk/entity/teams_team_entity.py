...
class TeamsTeamEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TeamsTeamEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> TeamsTeam:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.TeamsTeam().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: TeamsTeamCreateData, ctrl=None) -> TeamsTeam:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: TeamsTeamUpdateData, ctrl=None) -> TeamsTeam:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
