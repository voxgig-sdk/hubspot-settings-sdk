...
func TestTeamsTeamMemberEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.TeamsTeamMember(nil)
		if ent == nil {
			t.Fatal("expected non-nil TeamsTeamMemberEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := teams_team_memberBasicSetup(nil)
		...
		for _, _op := range []string{"create"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "teams_team_member." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HUBSPOT_SETTINGS_TEST_TEAMS_TEAM_MEMBER_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		teamsTeamMemberRef01Data = core.ToMapAny(entityData(teamsTeamMemberRef01DataResult))
		if teamsTeamMemberRef01Data == nil {
...
func teams_team_memberBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "teams_team_member", "TeamsTeamMemberTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read teams_team_member test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse teams_team_member test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("HUBSPOT_SETTINGS_TEST_TEAMS_TEAM_MEMBER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HUBSPOT_SETTINGS_TEST_TEAMS_TEAM_MEMBER_ENTID"])
