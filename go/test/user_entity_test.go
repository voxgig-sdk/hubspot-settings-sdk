...
func TestUserEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HUBSPOT_SETTINGS_TEST_USER_ENTID JSON to run live")
			return
		}
...
func userBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"user01", "user02", "user03", "2026_0901", "2026_0902", "2026_0903"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("HUBSPOT_SETTINGS_TEST_USER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HUBSPOT_SETTINGS_TEST_USER_ENTID"])
