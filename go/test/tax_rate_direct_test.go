...
func TestTaxRateDirect(t *testing.T) {
	t.Run("direct-list-tax_rate", func(t *testing.T) {
		setup := tax_rateDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-tax_rate", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "tax-rates/2026-09/tax-rates",
			"method": "GET",
	...
	t.Run("direct-load-tax_rate", func(t *testing.T) {
		setup := tax_rateDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-tax_rate", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "tax-rates/2026-09/tax-rates/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type tax_rateDirectSetupResult struct {
...
func tax_rateDirectSetup(mockres any) *tax_rateDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"HUBSPOT_SETTINGS_TEST_TAX_RATE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["HUBSPOT_SETTINGS_TEST_TAX_RATE_ENTID"]; ok {
		...
		return &tax_rateDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &tax_rateDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
