...
class TeamsBatchResponseTeamMemberEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = HubspotSettingsSDK::test(null, null);
        $ent = $testsdk->TeamsBatchResponseTeamMember(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = teams_batch_response_team_member_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "teams_batch_response_team_member." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HUBSPOT_SETTINGS_TEST_TEAMS_BATCH_RESPONSE_TEAM_MEMBER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $this->assertNotNull($teams_batch_response_team_member_ref01_data);
...
function teams_batch_response_team_member_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/teams_batch_response_team_member/TeamsBatchResponseTeamMemberTestData.json';
    ...
    $entid_env_raw = getenv("HUBSPOT_SETTINGS_TEST_TEAMS_BATCH_RESPONSE_TEAM_MEMBER_ENTID");
