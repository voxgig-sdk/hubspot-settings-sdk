...
class UserEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HUBSPOT_SETTINGS_TEST_USER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function user_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["user01", "user02", "user03", "2026_0901", "2026_0902", "2026_0903"] as $k) {
    ...
    $entid_env_raw = getenv("HUBSPOT_SETTINGS_TEST_USER_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["HUBSPOT_SETTINGS_TEST_USER_ENTID"]);
