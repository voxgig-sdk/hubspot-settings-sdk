...
// HubspotSettings SDK retry feature
...
class HubspotSettingsRetryFeature extends HubspotSettingsBaseFeature
{
    ...
    public function init(HubspotSettingsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HubspotSettingsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HubspotSettingsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HubspotSettingsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
