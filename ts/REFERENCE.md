# HubspotSettings TypeScript SDK Reference

Complete API reference for the HubspotSettings TypeScript SDK.
...
| Action | Route | Call |
| --- | --- | --- |
| `update_visibility` | `/settings/currencies/2026-09/exchange-rates/update-visibility` | `client.ExchangeRate().create({ $action: 'update_visibility', ... })` |

An action returns that action's OWN response, which is not necessarily a
ExchangeRate record — check the API definition for its shape.
...
| `links` | `Record<string, any>` | No | The link to the next page with exchange rates. |
