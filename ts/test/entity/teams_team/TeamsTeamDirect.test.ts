...
describe('TeamsTeamDirect', async () => {
  ...
  test('direct-load-teams_team', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-teams_team', setup.live)) return
    if (skipIfMissingIds(t, setup, ["team_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_SETTINGS_TEST_TEAMS_TEAM_ENTID']
