...
describe('MulticurrencyExchangeRateDirect', async () => {
  ...
  test('direct-load-multicurrency_exchange_rate', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-multicurrency_exchange_rate', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_SETTINGS_TEST_MULTICURRENCY_EXCHANGE_RATE_ENTID']
