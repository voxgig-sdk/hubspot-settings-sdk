...
describe('MulticurrencyCollectionResponseExchangeRateForwardPagingDirect', async () => {
  ...
  test('direct-list-multicurrency_collection_response_exchange_rate_forward_paging', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-multicurrency_collection_response_exchange_rate_forward_paging', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_SETTINGS_TEST_MULTICURRENCY_COLLECTION_RESPONSE_EXCHANGE_RATE_FORWARD_PAGING_ENTID']
