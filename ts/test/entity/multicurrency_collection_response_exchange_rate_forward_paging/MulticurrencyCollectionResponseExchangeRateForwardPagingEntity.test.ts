...
describe('MulticurrencyCollectionResponseExchangeRateForwardPagingEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'multicurrency_collection_response_exchange_rate_forward_paging.' + op, live)) return
    }
    ...
    let multicurrency_collection_response_exchange_rate_forward_paging_ref01_data = Object.values(setup.data.existing.multicurrency_collection_response_exchange_rate_forward_paging)[0] as any
    ...
    const multicurrency_collection_response_exchange_rate_forward_paging_ref01_match: any = {}

    const multicurrency_collection_response_exchange_rate_forward_paging_ref01_list = (await multicurrency_collection_response_exchange_rate_forward_paging_ref01_ent.list(multicurrency_collection_response_exchange_rate_forward_paging_ref01_match)).map((e: any) => e.data())
