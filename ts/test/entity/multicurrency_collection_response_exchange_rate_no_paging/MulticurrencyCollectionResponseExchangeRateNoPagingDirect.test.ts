...
describe('MulticurrencyCollectionResponseExchangeRateNoPagingDirect', async () => {
  ...
  test('direct-list-multicurrency_collection_response_exchange_rate_no_paging', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-multicurrency_collection_response_exchange_rate_no_paging', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_SETTINGS_TEST_MULTICURRENCY_COLLECTION_RESPONSE_EXCHANGE_RATE_NO_PAGING_ENTID']
