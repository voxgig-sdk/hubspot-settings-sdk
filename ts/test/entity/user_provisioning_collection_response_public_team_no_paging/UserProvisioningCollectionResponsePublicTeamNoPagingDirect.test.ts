...
describe('UserProvisioningCollectionResponsePublicTeamNoPagingDirect', async () => {
  ...
  test('direct-list-user_provisioning_collection_response_public_team_no_paging', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-user_provisioning_collection_response_public_team_no_paging', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_SETTINGS_TEST_USER_PROVISIONING_COLLECTION_RESPONSE_PUBLIC_TEAM_NO_PAGING_ENTID']
