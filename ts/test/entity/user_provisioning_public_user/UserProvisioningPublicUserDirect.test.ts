...
describe('UserProvisioningPublicUserDirect', async () => {
  ...
  test('direct-load-user_provisioning_public_user', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-user_provisioning_public_user', setup.live)) return
    if (skipIfMissingIds(t, setup, ["user_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_SETTINGS_TEST_USER_PROVISIONING_PUBLIC_USER_ENTID']
