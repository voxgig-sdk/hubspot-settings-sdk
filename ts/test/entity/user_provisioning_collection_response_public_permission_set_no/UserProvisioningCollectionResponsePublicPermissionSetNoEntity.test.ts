...
describe('UserProvisioningCollectionResponsePublicPermissionSetNoEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'user_provisioning_collection_response_public_permission_set_no.' + op, live)) return
    }
    ...
    let user_provisioning_collection_response_public_permission_set_no_ref01_data = Object.values(setup.data.existing.user_provisioning_collection_response_public_permission_set_no)[0] as any
    ...
    const user_provisioning_collection_response_public_permission_set_no_ref01_match: any = {}

    const user_provisioning_collection_response_public_permission_set_no_ref01_list = (await user_provisioning_collection_response_public_permission_set_no_ref01_ent.list(user_provisioning_collection_response_public_permission_set_no_ref01_match)).map((e: any) => e.data())
