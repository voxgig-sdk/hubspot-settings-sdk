...
describe('UserProvisioningCollectionResponsePublicPermissionSetNoDirect', async () => {
  ...
  test('direct-list-user_provisioning_collection_response_public_permission_set_no', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-user_provisioning_collection_response_public_permission_set_no', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_SETTINGS_TEST_USER_PROVISIONING_COLLECTION_RESPONSE_PUBLIC_PERMISSION_SET_NO_ENTID']
