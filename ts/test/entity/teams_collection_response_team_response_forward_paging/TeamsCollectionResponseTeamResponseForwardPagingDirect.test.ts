...
describe('TeamsCollectionResponseTeamResponseForwardPagingDirect', async () => {
  ...
  test('direct-list-teams_collection_response_team_response_forward_paging', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-teams_collection_response_team_response_forward_paging', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_SETTINGS_TEST_TEAMS_COLLECTION_RESPONSE_TEAM_RESPONSE_FORWARD_PAGING_ENTID']
