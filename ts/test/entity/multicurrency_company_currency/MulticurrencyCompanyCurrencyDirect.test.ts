...
describe('MulticurrencyCompanyCurrencyDirect', async () => {
  ...
  test('direct-load-multicurrency_company_currency', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-multicurrency_company_currency', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_SETTINGS_TEST_MULTICURRENCY_COMPANY_CURRENCY_ENTID']
