...
describe('MulticurrencyCentralExchangeRatesInformationDirect', async () => {
  ...
  test('direct-load-multicurrency_central_exchange_rates_information', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-multicurrency_central_exchange_rates_information', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_SETTINGS_TEST_MULTICURRENCY_CENTRAL_EXCHANGE_RATES_INFORMATION_ENTID']
